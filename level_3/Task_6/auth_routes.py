...
from mongo_config import UserCollection
...
async def RegisterUser(user: RegisterUserRequest):
...
async def LoginUser(user: LoginUserRequest):
    # Find user by email
    existing = await UserCollection.find_one({"Email": user.Email})
...
async def RequestPasswordReset(payload: PasswordResetRequest):
...
async def ConfirmPasswordReset(payload: NewPasswordRequest):
